---
title: "PreSScArthritis_vs_SScArthritis"
...
**Analysis:**
- Binary limma: Diagnosis (veSSc vs SSc) + Immunosuppression covariate
- Filter for AveExpr > 0, recalculate FDR
- Volcano plot and boxplots for significant hits

**Interpretation:**
- Positive logFC = higher in veSSc
...
# Load Olink data
olink <- read_parquet("/Users/astrid/Desktop/PhD /RESOLVE_SerumProteomics/data/P2024-199-OLI_80_NPX_2024-10-10.parquet")

# Load metadata (sheet 2)
metadata <- read_excel("/Users/astrid/Desktop/PhD /RESOLVE_SerumProteomics/data/Metadata_veSSC_vs_SSc_arthritis_final.xlsx", sheet = 2)


# Option 2: If data is relative to where the .Rmd file is
olink <- read_parquet("../data/P2024-199-OLI_80_NPX_2024-10-10.parquet")
...
cat("\nDiagnosis breakdown:\n")
print(table(filtered_df$Diagnosis[!duplicated(filtered_df$patient)]))
```
...
cat("\nDiagnosis:\n")
print(table(filtered_metadata$Diagnosis))
cat("\nImmunosuppression:\n")
...
print(table(filtered_metadata$Visit_Year, filtered_metadata$Diagnosis))
```
...
# 9. Binary Analysis: veSSc vs SSc
...
cat("FDR < 0.3:", sum(results_filtered$adj.P.Val < 0.3), "proteins\n")
```
...
cat("Higher in veSSc (positive logFC):", nrow(hits_up), "\n")
if(nrow(hits_up) > 0) cat("  ", paste(hits_up$Assay, collapse = ", "), "\n\n")

cat("Higher in SSc (negative logFC):", nrow(hits_down), "\n")
...
cat("Files:\n")
cat("  - results_veSSc_vs_SSc_all.csv\n")
cat("  - results_veSSc_vs_SSc_FDR02.csv\n")
cat("  - volcano_veSSc_vs_SSc.pdf\n")
cat("  - boxplots_veSSc_vs_SSc_top20.pdf\n")
```
